...
describe('Utils', () => {
  describe('getURL', () => {
    it('should build URL for sending funds', () => {
      ...
      expect(getURL({ amount, recipient, kind: 'send' }))
      .toBe(`lisk://main/transactions/send?recipient=${recipient}&amount=${amount}`);
    });
      it('should build URL for delegate voting', () => {
        ...
        expect(getURL({ votes, unvotes, kind: 'vote' }))
        .toBe(`lisk://main/voting/vote?unvotes=${unvotes}&votes=${votes}`);
      })
  })
})
